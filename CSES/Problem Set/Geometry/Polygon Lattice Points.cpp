...
ll check(Point p1,Point p2){
    return gcd(p1.X-p2.X,p1.Y-p2.Y);
}
int main(){
    ...
    ar+=puntos[n-1].X*puntos[0].Y-puntos[0].X*puntos[n-1].Y;
    cant+=check(puntos[n-1],puntos[0]);
    for(int i=1;i<n;i++){
        ar+=puntos[i-1].X*puntos[i].Y-puntos[i].X*puntos[i-1].Y;
        cant+=check(puntos[i-1],puntos[i]);
    }
    cout << (abs(ar)-cant)/2+1 << ' ' << cant;
}